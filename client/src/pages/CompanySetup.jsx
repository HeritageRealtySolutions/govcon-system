...
function missingFields(p) {
  const labels = { company_name:'Company Name', uei_number:'UEI Number', cage_code:'CAGE Code', bonding_capacity:'Bonding Capacity', years_in_business:'Years in Business', address:'Address', phone:'Phone', email:'Email', poc_name:'Point of Contact' };
...
export default function CompanySetup() {
  ...
  useEffect(() => {
    authFetch(`${BASE_URL}/api/company`).then(r => r.json())
  ...
  async function save() {
    ...
    try {
      const r = await authFetch(`${BASE_URL}/api/company`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(profile) });
      const d = await r.json();
      if (d.error) alert(d.error); else { setSaved(true); setTimeout(() => setSaved(false), 3000); }
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><span className="w-6 h-6 border-2 border-gray-200 border-t-gray-900 rounded-full animate-spin"/></div>;
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        ...
        {missing.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {missing.map(m => <span key={m} className="bg-gray-100 text-gray-500 text-xs px-2 py-0.5 rounded-full">Missing: {m}</span>)}
          </div>
        )}
      ...
      {preview && (
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-5 font-mono text-xs space-y-1.5">
          ...
          {[
            ...
            <p key={k}><span className="text-gray-500">{k}:</span>{' '}<span className={v?.trim() ? 'text-white' : 'text-red-400'}>{v?.trim() || '[BLANK]'}</span></p>
          ))}
        </div>
      )}
      ...
      <Section icon="🏆" title="Certifications & Capabilities">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <label className="block text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1.5">Certifications <span className="text-gray-400 normal-case font-normal">— comma separated</span></label>
            <input value={profile.certifications} onChange={e => set('certifications', e.target.value)} placeholder="8(a), Small Business, Black-Owned, MBE, DBE" className={inputCls} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={profile.all_naics} onChange={e => set('all_naics', e.target.value)} placeholder="238210,238220,238160" className={inputCls} />
          </div>
          <div>
            <label className="block text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1.5">Bonding Capacity ($)</label>
            <input type="number" value={profile.bonding_capacity} onChange={e => set('bonding_capacity', e.target.value)} placeholder="500000" className={inputCls} />
          </div>
          <div>
            <label className="block text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1.5">Years in Business</label>
            <input type="number" value={profile.years_in_business} onChange={e => set('years_in_business', e.target.value)} placeholder="5" className={inputCls} />
          </div>
          <div>
            <label className="block text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1.5">Employees</label>
            <input type="number" value={profile.employee_count} onChange={e => set('employee_count', e.target.value)} placeholder="10" className={inputCls} />
          </div>
          <div className="sm:col-span-2">
