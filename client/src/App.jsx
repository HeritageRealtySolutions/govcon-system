...
// ── Inner layout — lives INSIDE BrowserRouter so useLocation works ──
function AppLayout() {
  const [collapsed, setCollapsed] = useState(false);
  const location = useLocation();
  const currentLabel = NAV.find(n => n.end ? location.pathname === n.to : location.pathname.startsWith(n.to))?.label ?? 'Dashboard';
    ...
    <div className="min-h-screen bg-slate-900 flex">
      ...
      <aside className={`${collapsed ? 'w-16' : 'w-60'} flex-shrink-0 bg-slate-950 border-r border-slate-800 flex flex-col transition-all duration-200 sticky top-0 h-screen`}>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {!collapsed && (
            <p className="text-slate-600 text-xs font-semibold uppercase tracking-widest px-3 mb-2">Main Menu</p>
          )}
          {NAV.map(n => (
            <NavLink
              key={n.to}
...
// ── Outer shell — just provides the router ──
