...
async function start() {
  ...
  app.get('/health', (req, res) => res.json({ status: 'ok', gemini: !!process.env.GEMINI_API_KEY }));

  // Cron endpoint — SAM + USASpending + SLED + Email alerts
  app.get('/cron/sync-opportunities', async (req, res) => {
    ...
    try {
      ...
      results.sled = await syncSLEDBids();
      console.log(`[CRON] SLED: ${results.sled.total_saved} saved`);
      ...
      console.log(`[CRON] NAICS: ${scores.length} codes refreshed`);

      // 4 — Recompetes
      const now = new Date(), window = new Date(now.getTime() + 180 * 86400000);
      const allRecompetes = [];
      ...
      for (let i = 0; i < deduped.length; i += 50) await supabase.from('recompetes').insert(deduped.slice(i, i + 50));
      results.recompetes = deduped.length;
      console.log(`[CRON] Recompetes: ${deduped.length} found`);

      // 5 — Email alerts
      try {
        const { sendDeadlineAlerts } = require('./services/emailAlerts');
        results.email = await sendDeadlineAlerts();
        console.log(`[CRON] Email: ${JSON.stringify(results.email)}`);
      } catch (emailErr) {
  ...
  app.use('/api/sled',          requireAuth, require('./routes/sled'));
